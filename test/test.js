...
const RaffleFrens = artifacts.require('./RaffleFrens')
...
require('chai')
    ...
    contract('RaffleFrens', ( [deployer, user1, user2] ) => {
        let contract = RaffleFrens
        let name = "Raffle Frens"
        let symbol = "RF"
        let BaseUri = "revealURI"
        let notreavealedUri = 

        beforeEach(async () => {
            contract = await RaffleFrens.new("Raffle Frens", "RF", "revealUri", "notRevealUri", "0x784348c1cB37C0A8e19D321Ea3072FDb46041b30")
        })

        describe('Sale is Active', () => {
            let result
            ...
            describe('Success', () => {
                ...
                it('is sale not active?', async () => {
                    expect(Boolean(await contract.getSaleState().to.eq(Boolean(false))))
                })
